...
class ForgotPasswordRequestView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({'message': 'If this email is registered, a reset code has been sent.', 'expires_in_seconds': 300}, status=status.HTTP_200_OK)
...
class UpdateEmailRequestView(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        verification = serializer.save()
        return Response({'message': f'Verification code sent to {verification.email}.', 'expires_in_seconds': 300}, status=status.HTTP_200_OK)
