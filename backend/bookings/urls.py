...
urlpatterns = [
    ...
    # Food order check-in gate — used by FoodAndDrinks.jsx before placing orders
    path("my-active/", views.MyActiveBookingView.as_view(), name="my-active-booking"),
    ...
    # ── Staff-only booking actions ────────────────────────────────────────────
    path("admin/<int:pk>/extend/", views.StaffExtendBookingView.as_view(), name="admin-extend"),

    # ── Checkout — StaffCheckoutAndCollectView (Option A) ─────────────────────
    # Replaces the old StaffCheckOutView. Now atomically:
    #   1. Creates a BALANCE_PAYMENT Payment record if accommodation balance > 0
    #   2. Transitions CHECKED_IN → CHECKED_OUT
    #   3. Generates ReviewToken + sends review email
    # Frontend (GuestCheckoutPage) settles food orders before calling this.
    path("admin/<int:pk>/checkout/", views.StaffCheckoutAndCollectView.as_view(), name="admin-checkout"),
]