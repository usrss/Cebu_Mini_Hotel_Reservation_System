/**
 * src/features/staff/housekeeping/HousekeepingDashboard.jsx
 *
 * Full housekeeping dashboard.
 *  - Housekeeping staff: see only their assigned tasks, Start/Done buttons
 *  - Admin/Manager: see all tasks, New Task form, Assign button per card
 *
 * Assignment flow:
 *   Admin clicks "Assign" on a card → inline dropdown appears →
 *   selects housekeeping staff → calls PATCH /api/staff/cleaning/<pk>/assign/
 *   → backend fires in-app notification to assigned staff member
 *   → assigned staff sees bell badge + task appears on their dashboard
 */

import { useState, useEffect, useCallback, useRef } from 'react';
import {
  Plus, Clock, AlertTriangle, CheckCircle2,
  RefreshCw, Sparkles, UserPlus, X,
} from 'lucide-react';
...
// One-way workflow enforced on the frontend.
// Backend also enforces this via ALLOWED_TRANSITIONS.
const TRANSITIONS = {
  dirty:    ['cleaning'],
  cleaning: ['clean'],
  clean:    [],
...
function getTimeDiff(endAt) {
  if (!endAt) return null;
  return new Date(endAt) - Date.now();
}
...
function CleaningTimer({ cleaningEndAt, status }) {
  ...
  const diff     = getTimeDiff(cleaningEndAt);
...
// Shown only to Admin/Manager inside the task card.
...
function AssignWidget({ task, hkStaff, onAssigned }) {
  ...
  if (!open) {
    return (
      <button
        className="hk-assign-btn"
        onClick={() => setOpen(true)}
        title="Assign to housekeeping staff"
      >
        <UserPlus size={11} />
  ...
  return (
    <div className="hk-assign-widget">
      <div className="hk-assign-row">
        <select
          className="sf-select hk-assign-select"
          value={selected}
        ...
        <button
          className="sf-task-btn sf-task-btn-green"
        ...
        <button
          className="sf-task-btn sf-task-btn-muted"
...
function SummaryCard({ label, value, accent, icon }) {
  return (
    <div className={`hk-summary-card hk-summary-card--${accent}`}>
...
function TaskCard({ task, onAction, onAssigned, actionBusy, canAct, canAssign, hkStaff }) {
  const isOverdue = task.status !== 'clean'
    && task.cleaning_end_at
    && new Date(task.cleaning_end_at) < new Date();
    ...
    <div className={`hk-task-card ${isOverdue ? 'hk-task-card--overdue' : ''} ${task.status === 'clean' ? 'hk-task-card--done' : ''}`}>
      ...
      <div className="hk-task-head">
        ...
        <div className="hk-task-badges">
          {isOverdue && (
            <span className="sf-badge hk-badge-overdue">
              <AlertTriangle size={9} /> Overdue
            </span>
          )}
          {task.status === 'clean' && (
            <span className="sf-badge hk-badge-done">
              <CheckCircle2 size={9} /> Done
            </span>
          )}
          <span className={`sf-badge ${STATUS_CLASS[task.status]}`}>
            {STATUS_LABELS[task.status]}
          </span>
          <span className={`sf-badge ${PRIORITY_CLASS[task.priority]}`}>
      ...
      <p className="hk-task-assigned">
        {task.assigned_to_name
          ? <>→ {task.assigned_to_name}</>
          : <span className="hk-unassigned">Unassigned</span>
        }
      </p>
      ...
      {task.status !== 'clean' && task.cleaning_started_at && (
        <div className="hk-cleaning-window">
          ...
          <div className="hk-window-row">
            ...
            <span className={`hk-window-value ${isOverdue ? 'hk-text-danger' : 'hk-text-amber'}`}>
              {formatDateTime(task.cleaning_end_at)}
            </span>
          </div>
        </div>
      )}
      ...
      {/* Assign widget — admin/manager only, not shown for completed tasks */}
      ...
      {/* Status action buttons — housekeeping + admin/manager */}
...
export default function HousekeepingDashboard() {
  ...
  // Load housekeeping staff and rooms for the create form (admin/manager only)
  useEffect(() => {
  ...
  <button className="sf-btn sf-btn-primary" onClick={() => setShowForm(!showForm)}>
            ...
            {!perms.canManageCleaning && perms.canAccessCleaning && (
              <span className="sf-badge sf-badge-gold">
        ...
        <div className="hk-summary-grid">
          <SummaryCard label="Pending"     value={pendingCount}    accent="red"                             icon={<Clock size={18} />} />
          <SummaryCard label="In Progress" value={inProgressCount} accent="amber"                           icon={<RefreshCw size={18} />} />
          <SummaryCard label="Done"        value={doneCount}       accent="green"                           icon={<CheckCircle2 size={18} />} />
          <SummaryCard label="Overdue"     value={overdueCount}    accent={overdueCount > 0 ? 'overdue' : 'muted'} icon={<AlertTriangle size={18} />} />
        </div>
            ...
            <form onSubmit={handleCreate}>
              <div className="sf-form-row">
                <div className="sf-form-group">
                  ...
                  <select
                    className="sf-select" value={form.room}
                  ...
                  >
                    ...
                    {rooms.map(r => (
                      <option key={r.id} value={r.id}>
                        #{r.room_number} — {r.room_type_display || r.room_type}
        ...
        <div className="sf-filter-bar">
          ...
          {[
            { value: '',         label: 'All',         count: tasks.length,   countClass: '' },
            { value: 'dirty',    label: 'Pending',     count: pendingCount,    countClass: 'hk-filter-count--red' },
            { value: 'cleaning', label: 'In Progress', count: inProgressCount, countClass: 'hk-filter-count--amber' },
            { value: 'clean',    label: 'Done',        count: doneCount,       countClass: 'hk-filter-count--green' },
          ].map(opt => (
            <button
              key={opt.value}
            ...
            >
              {opt.label}
              {opt.count > 0 && (
                <span className={`hk-filter-count ${opt.countClass}`}>{opt.count}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="hk-empty">
            ...
            <p>
              {statusFil
                ? `No ${STATUS_LABELS[statusFil] ?? statusFil} tasks.`
                : 'No cleaning tasks at the moment.'}
            </p>
            {statusFil && (
              <button className="sf-btn" onClick={() => setStatusFil('')}>Show all</button>
            )}
          </div>
        ) : (
