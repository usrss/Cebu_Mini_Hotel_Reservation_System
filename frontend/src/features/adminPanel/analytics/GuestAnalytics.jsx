...
export default function GuestAnalytics({ period }) {
  ...
  useEffect(() => {
    ...
    analyticsApi.report('guests', chartPeriod)
      .then((res) => {
        // API wraps the result: { report_type, period, data: { summary, rows } }
        const data = res?.data ?? res;
        console.log('[GuestAnalytics] raw response:', res);
        console.log('[GuestAnalytics] parsed data:', data);
        setReport(data);
      })
      .catch((err) => console.error('[GuestAnalytics] fetch failed:', err))
  ...
  // "Total Guests" — prefer new_registrations (date_joined count),
  // fall back to registered_bookings, then sum of new+returning
  const totalGuests = summary.new_registrations
                   ?? summary.registered_bookings
                   ?? (newG + returning)
                   ?? null;
  ...
  return (
    <>
      <div className="an-grid-4">
        {[
          { label: 'Total Guests',      value: loading ? '…' : (totalGuests ?? '—'), color: '#3B5BDB' },
          { label: 'New Guests',        value: loading ? '…' : (newG        || '—'), color: '#6EE7B7' },
          { label: 'Returning Guests',  value: loading ? '…' : (returning   || '—'), color: '#93C5FD' },
          { label: 'Avg Stay (nights)', value: loading ? '…' : (summary.avg_stay_nights ?? '—'), color: '#FCD34D' },
        ].map((k, i) => (
        ...
        <div className="an-card">
          <div className="an-card-header">
            <div>
              ...
              <h3 className="an-card-title">Guest Registrations</h3>
            </div>
          ...
          {trendData.length ? (
            ...
            <div className="an-empty">
              {loading ? 'Loading…' : 'No guest trend data for this period.'}
            </div>
          )}
        </div>
